...
var Starport = defineComponent({
  ...
  setup(props, ctx) {
    ...
    return () => {
      ...
      if (!(0, core_exports.isObject)(component) || isVNode(component)) {
        component = {
          render() {
            return slots;
          }
...
function useElementBounding(target) {
  ...
  const root = core_exports.isClient ? document.documentElement || document.body : void 0;
  function update() {
    if (!core_exports.isClient)
      return;
    const el = (0, core_exports.unrefElement)(target);
    if (!el)
      return;
  ...
  const raf = (0, core_exports.useRafFn)(update, { immediate: false });
  function listen() {
    if (!core_exports.isClient)
      return;
    update();
